...
import { nacionalidadesMask, listadoBancos, CrearBancoModel } from 'src/app/administrador/proyectos/models/project.models';
...
@Component({
  ...
  styleUrls: ['./crear-banco.component.scss']
})
export class CrearBancoComponent implements OnInit {
  ...
  constructor(
    ...
    private solicitanteNgrxSvc: NgrxServicesService
  ) {
    this.registrarFormulario();
  }

  ngOnInit(): void {
    this.bancoState$ = this.solicitanteNgrxSvc.SeleccionarEliminarRegistrarBanco();
  ...
  guardarBanco() {
    ...
    this.solicitanteNgrxSvc.cargarRegistroBanco(banco);
  }
